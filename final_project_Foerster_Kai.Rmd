...
# Summary statistics of BMZ and UNICEF Data

### Create data subsets
Here I am creating data subsets for BMZ and UNICEF data

```{r filter BMZ UNICEF data, results='hide'}

bmz_data <- aiddata %>% filter(reporting=="Federal Ministry for Economic Cooperation and Development (BMZ)")

unicef_data <- aiddata %>% filter(reporting=="UNICEF")

```

### BMZ by sector

The graph shows the share of BMZ projects by sector (where the share is above 50 projects, about 0.5% of the total number of projects). We can observe that the most projects involve civil society, followed by agriculture, multisector, water&sanitation and basic health. Note, that some categories could be merged together or are not so informative. For example "other multi-sector" is not informative and "Health General" could be merged with "Basic Health". The operations would change the chart more.

```{r , results='hide'}
# Turn BMZ data into data.table
bmz_data_sector <- data.table(bmz_data)

# Set keys for the data.table
setkeyv(bmz_data_sector, c("aid", "sector_code"))

# Remove duplicates by keeping the first occurrence of each unique aid and sector_code pair
bmz_data_sector <- bmz_data_sector[!duplicated(bmz_data_sector, by = c("aid", "sector_code"))]

# Create the share column
bmz_data_sector[, share := sector_percent / 100]

# Summarize the weighted number of projects per sector
bmz_data_sectorGroup <- bmz_data_sector[, .(weighted_projects = sum(share)), by = sector_group]

# Order the data by weighted_projects in descending order
bmz_data_sectorGroup <- bmz_data_sectorGroup[order(-weighted_projects)]

# Calculate the percentage of total projects
total_projects <- sum(bmz_data_sectorGroup$weighted_projects)
bmz_data_sectorGroup[, percentage := (weighted_projects / total_projects) * 100]

# Create the bar graph with percentages above the bars
ggplot(bmz_data_sectorGroup[weighted_projects > 50], aes(x = reorder(sector_group, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 2) +
  labs(title = "Weighted Number of Projects per Sector",
       x = "Sector Group",
       y = "Weighted Number of Projects") +
  theme_minimal() +
...
So in a second step, I aggreagegated the sector groups further:

```{r , results='hide'}

# Create a data frame for the breakdown table
breakdown_table <- data.frame(
  aggregate_category = c(
    rep("Government and Civil Society", 4),
    rep("Economic Development", 9),
    rep("Infrastructure", 4),
    rep("Health", 4),
    rep("Emergency and Disaster Management", 4),
    rep("Environment (incl. agriculture and fishing)", 4),
    rep("Energy", 4),
    rep("Education", 4),
    rep("Admin", 1),
    rep("Miscellaneous", 1)
...
# Merge the lookup table with bmz_data_sector to create the aggregate_category column
bmz_data_sector <- merge(bmz_data_sector, breakdown_table, by = "sector_group", all.x = TRUE)

# Summarize the weighted number of projects per new sector group
bmz_data_sectorGroup <- bmz_data_sector[, .(weighted_projects = sum(share)), by = aggregate_category]
...
# Create the bar graph with percentages above the bars
ggplot(bmz_data_sectorGroup[weighted_projects > 50,], aes(x = reorder(aggregate_category, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 3) +
  labs(title = "Weighted Number of Projects per Sector",
       x = "Aggregate Category",
       y = "Weighted Number of Projects") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Here is the breakdown of categories by sector_group:

```{r}

# Convert the breakdown table to a visually appealing format
breakdown_table2 <- breakdown_table %>%
  group_by(aggregate_category) %>%
  mutate(aggregate_category = ifelse(row_number() == 1, aggregate_category, ""))

# Display the breakdown table using knitr for Rmarkdown
knitr::kable(breakdown_table2, caption = "Breakdown of Sector Groups into Aggregate Categories")

```

### UNICEF by sector

The graph shows the share of BMZ projects by sector (where the share is above 50 projects, about 0.5% of the total number of projects). We can observe that the most projects involve administrative costs. Education, against the intution, only comes fourth in terms of share.

```{r , results='hide'}
# Turn UNICEF data into data.table
unicef_data_sector <- data.table(unicef_data)

# Set keys for the data.table
setkeyv(unicef_data_sector, c("aid", "sector_code"))

# Remove duplicates by keeping the first occurrence of each unique aid and sector_code pair
unicef_data_sector <- unicef_data_sector[!duplicated(unicef_data_sector, by = c("aid", "sector_code"))]

# Create the share column
unicef_data_sector[, share := sector_percent / 100]

# Summarize the weighted number of projects per sector
unicef_data_sectorGroup <- unicef_data_sector[, .(weighted_projects = sum(share)), by = sector_group]

# Order the data by weighted_projects in descending order
unicef_data_sectorGroup <- unicef_data_sectorGroup[order(-weighted_projects)]

# Calculate the percentage of total projects
total_projects <- sum(unicef_data_sectorGroup$weighted_projects)
unicef_data_sectorGroup[, percentage := (weighted_projects / total_projects) * 100]

# Create the bar graph with percentages above the bars
ggplot(unicef_data_sectorGroup[weighted_projects > 50], aes(x = reorder(sector_group, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 2) +
  labs(title = "Weighted Number of Projects per Sector",
       x = "Sector Group",
...
In a second step, I aggreagegated the sector groups further, showing that the highest share is administrative costs, followed by health and infrastructure.

```{r , results='hide'}
#unicef_data_sector <- as.data.table(unicef_data_sector)
#unicef_data_sector[, sector_group := trimws(sector_group)]
#breakdown_table <- as.data.table(breakdown_table)
# Merge the lookup table with bmz_data_sector to create the aggregate_category column
unicef_data_sector <- left_join(unicef_data_sector, breakdown_table, by = "sector_group")

test <- unicef_data_sector[aggregate_category=="",]

# Summarize the weighted number of projects per new sector group
unicef_data_sectorGroup <- unicef_data_sector[, .(weighted_projects = sum(share)), by = aggregate_category]
...
# Create the bar graph with percentages above the bars
ggplot(unicef_data_sectorGroup[weighted_projects > 50,], aes(x = reorder(aggregate_category, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 3) +
  labs(title = "Weighted Number of Projects per Sector",
       x = "Aggregate Category",
...
The graph shows the share of BMZ projects by country (where the share is above 50 projects, about 0.5% of the total number of projects). We can observe that there is a concentration of projects in some countries including S. Africa, Kenya, DRC and Ethopia.
```{r , results='hide'}
# Turn BMZ data into data.table
bmz_data_ctr <- data.table(bmz_data)

# Set keys for the data.table
setkeyv(bmz_data_ctr, c("aid", "country_code"))

# Remove duplicates by keeping the first occurrence of each unique aid and country_code pair
bmz_data_ctr <- bmz_data_ctr[!duplicated(bmz_data_ctr, by = c("aid", "country_code"))]

# Create the share column
bmz_data_ctr[, share := country_percent / 100]

# Summarize the weighted number of projects per country
bmz_data_ctrGroup <- bmz_data_ctr[, .(weighted_projects = sum(share)), by = country_code]

# Order the data by weighted_projects in descending order
bmz_data_ctrGroup <- bmz_data_ctrGroup[order(-weighted_projects)]

# Calculate the percentage of total projects
total_projects <- sum(bmz_data_ctrGroup$weighted_projects)
bmz_data_ctrGroup[, percentage := (weighted_projects / total_projects) * 100]

# Create the bar graph with percentages above the bars
ggplot(bmz_data_ctrGroup[weighted_projects > 50], aes(x = reorder(country_code, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 2) +
  labs(title = "Weighted Number of Projects per Country",
       x = "Country",
       y = "Weighted Number of Projects") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
The same graph for UNICEF shows a much less concentated picture.

```{r , results='hide'}
# Turn BMZ data into data.table
unicef_data_ctr <- data.table(unicef_data)

# Set keys for the data.table
setkeyv(unicef_data_ctr, c("aid", "country_code"))

# Remove duplicates by keeping the first occurrence of each unique aid and country_code pair
unicef_data_ctr <- unicef_data_ctr[!duplicated(unicef_data_ctr, by = c("aid", "country_code"))]

# Create the share column
unicef_data_ctr[, share := country_percent / 100]

# Summarize the weighted number of projects per country
unicef_data_ctrGroup <- unicef_data_ctr[, .(weighted_projects = sum(share)), by = country_code]

# Order the data by weighted_projects in descending order
unicef_data_ctrGroup <- unicef_data_ctrGroup[order(-weighted_projects)]

# Calculate the percentage of total projects
total_projects <- sum(unicef_data_ctrGroup$weighted_projects)
unicef_data_ctrGroup[, percentage := (weighted_projects / total_projects) * 100]

# Create the bar graph with percentages above the bars
ggplot(unicef_data_ctrGroup[weighted_projects > 50], aes(x = reorder(country_code, -weighted_projects), y = weighted_projects)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), vjust = -0.5, size = 2) +
  labs(title = "Weighted Number of Projects per Country",
       x = "Country",
...
### Distribution of Committment by BMZ and UNICEF
...
[UNICEF](https://www.iatiregistry.org/publisher/unicef): Monthly updated files. Check  [here](https://open.unicef.org/transparency-tool/) for more details. UNICEF claims to completeness of data reported to d-portal, although provisional. Offical numbers can be found in DAC.
