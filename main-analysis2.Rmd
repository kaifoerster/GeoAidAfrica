...
projectByInstitution <- ggplot(prepared_data, aes(x = reporting, y = Number_of_Projects, fill = reporting)) +
  ...
  geom_text(aes(label = Number_of_Projects), vjust = -0.3, color = "black", size = 2.3) +  # Add text labels above bars
  theme_minimal() +
  labs(x = "Institution",
  ...
  scale_y_continuous(labels = label_comma(), limits = c(NA, max(prepared_data$Number_of_Projects) * 1.1))  # Extend y-axis and format labels
...
# Summarize data to count number of projects by location predicsion
project_counts <- aiddata %>%
